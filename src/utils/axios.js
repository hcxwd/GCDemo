  ...
  if (error.response) {
    ...
    switch (error.response.status) {
      case 400:
        ...
        return Toast.error('请求参数（data）格式错误（' + error.config.method + error.config.url + '）');
        break;
      case 401:
      case 504:
      ...
      case 404:
        ...
        return Toast.error('请求 URL 格式错误（' + error.config.url + '）');
        break;
      case 405:
        // 405（Method Not Allowed）：请求 Method 格式错误；提示错误消息
        return Toast.error('请求 Method 格式错误（' + error.config.url + '）');
        break;
      case 406:
        // 406（Not Acceptable）：请求 Content-Type 格式错误
        return Toast.error('请求 Content-Type 格式错误（' + error.config.url + '）');
        break;
      case 408:
        // 408（Request Timeout）：请求超时
        return Toast.warning('请求超时（' + error.config.url + '）');
        break;
    }
    ...
    if (error.message === 'timeout of ' + error.config.timeout + 'ms exceeded') {
      return Toast.warning('请求超时，请刷新页面重新请求！');
    }
    // 2 ）网络错误
